    ...
    return new Promise((resolve, reject) => {
        ...
        dumpProcess.on('exit', (code) => {
            if (code === 0) {
                ...
                reject({ success: false, error: `Error al terminar pg_dump: <<${dumpProcess.spawnargs}>>, falló con código <<${code}>>, Detalles: <<${stderr.replace(/[\r\n]+/g, '')}>>` });
            }
        });

        dumpProcess.on('error', (err) => {
            reject({ success: false, error: `Error ejecución pg_dump: <<${dumpProcess.spawnargs}>>, falló con error: <<${err.message}>>` });
        });
    });
}
