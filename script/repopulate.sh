...
db_user=$6
FILTER="$SOURCE_PATH/filter.filter"
if [ -f $FILTER ]; then
  echo "hay filter"
  if [ -x $FILTER ]; then
    echo "filtro detectado $FILTER"
  fi
else
  FILTER=""
  echo "no filter"
...
then
  echo "repopulating"
  sed -e "s/${ori_install_dump_db_owner}/${install_dump_db_owner}/g ; s/${ori_db_user}/${db_user}/g $(cat $FILTER) " $DUMP_FILE | \
